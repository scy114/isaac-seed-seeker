...
当前版本不搜索一层宝箱房、Boss、Boss 奖励及其他房间内容。除实验性疗法外，开局道具对最终面板的完整影响也不在当前计算范围内。

内置 Profile 不适用于改变解锁状态、物品池或生成流程的大型内容 Mod。游戏更新即使没有修改随机数算法，也可能改变运行时数据，因此新版本仍需重新验证 Profile。

离线结果经过固定种子和游戏内观察数据回归验证；若计算结果与游戏表现不一致，以游戏内结果为准。
