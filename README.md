...
- WebUI 可组合筛选口袋物类型、卡牌 ID、胶囊效果 ID、主动/被动道具 ID，以及基础血量和六项随机属性区间。
...
道具名称层也已完成数据设计与固定快照：仓库内的 J460 离线目录包含中英文名、俗称、拼音、ID 和当前伊甸池可用性，并处理金色饰品与大胶囊 ID 归一化。现有 WebUI 仍使用 ID 输入，下一阶段会把目录接成可搜索的多选输入框；设计、来源与更新方式见 [`docs/item-catalog.md`](docs/item-catalog.md)。
