...
- 尚未搜索一层宝箱房、Boss、Boss 奖励及其他房间内容；
- 除实验性疗法外，尚未模拟开局道具对最终面板的完整影响；
- 内置 Profile 不适用于改变解锁状态、物品池或生成流程的大型内容 Mod；
- 游戏更新可能改变运行时数据，即使随机数算法本身保持不变，也需要重新验证 Profile；
- 离线结果经过固定种子和游戏内观察数据回归验证，但游戏内实际结果仍是最终判定依据。
...
项目许可证尚未确定。在许可证文件正式发布前，本仓库代码不授予复制、修改或再分发许可。
