...
页面内的“挑战自我”使用独立的严格规则：钱、钥匙和炸弹必须全部为 `0`，口袋物只能为空或坏胶囊效果 `1`、`6`、`11`、`13`、`15`、`17`，坏胶囊候选权重为无口袋物候选的 3 倍。主动道具限定为 `36`、`39`、`41`、`177`、`287`、`290`、`294`、`325`、`475`、`480`、`481`、`582`。被动为 `561` 或 `697` 时，要求移速低于 `1.0`、伤害低于 `3.0`、射速低于 `2.0`；被动为实验性疗法 `240` 时，要求结算后伤害低于 `2.0`、射速低于 `1.5`。满足黄针分支的候选使用更高基础权重，使稀有的极差黄针在进入当日候选窗口后优先被抽中。首次生成及每次换种都会扫描完整的 `2^32` 种子空间，页面会在扫描期间显示等待提示；当天已经生成的结果保存在浏览器中，再次进入时直接读取。
