...
“每日毒种”使用独立的每日种子序列。当前粗筛要求移速低于 `1.0`、射速低于 `2.5`、伤害低于 `3.0`，并且初始炸弹为 `0`。基础道具池只允许 Q0 主动和 Q0 被动，并额外放行主动道具 `33`、`38`、`45`、`298`、`522`、`639`、`729` 与被动道具 `149`、`222`、`329`、`529`、`561`。道具 `19`、`59`、`137`、`161`、`482` 被排除，道具 `721` 的候选权重为普通种子的 70%。页面同样支持换种和按需揭晓。

页面内的“挑战自我”使用独立的严格规则：钱、钥匙和炸弹必须全部为 `0`，口袋物只能为空或坏胶囊效果 `1`、`6`、`11`、`13`、`15`、`17`，坏胶囊候选权重为无口袋物候选的 3 倍。主动道具限定为 `36`、`39`、`41`、`177`、`287`、`290`、`294`、`325`、`475`、`480`、`481`、`582`。被动为 `561` 或 `697` 时，要求移速低于 `1.0`、伤害低于 `3.0`、射速低于 `2.0`；被动为实验性疗法 `240` 时，要求结算后伤害低于 `2.0`、射速低于 `1.5`。抽取时先按黄针 `40%`、消失的胞胎 `30%`、杏仁奶 `30%` 选择被动分支，再在该分支内部按坏胶囊 3 倍权重选择种子，避免稀有黄针权重挤占另外两类挑战。程序只在本机尚无候选池时扫描一次完整的 `2^32` 种子空间，并将结果写入 `%LOCALAPPDATA%\IsaacSeedSeeker\cache`；此后的每日抽取和换种都直接复用缓存。
