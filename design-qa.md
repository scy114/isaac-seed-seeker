...
- 主副标题下移、空白初始状态桌面截图：`output/design-audit/41-lowered-title-empty-default-desktop.png`
- 主副标题下移、空白初始状态中等宽度截图：`output/design-audit/42-lowered-title-empty-default-medium.png`
- 主副标题下移、空白初始状态窄屏截图：`output/design-audit/43-lowered-title-empty-default-mobile.png`
- 下移前 / 空白通用状态同尺寸对照：`output/design-audit/44-title-position-default-state-before-after.png`
- 状态：默认空白筛选条件，离线目录、游戏图标、LanaPixel 字体和地下室背景均已载入，页面位于顶部；目标 169 仅保留为回归验证样例。
...
- 主副标题下移前后的桌面截图均为 `1265 × 720 px`，对应 `1280 × 720` CSS 视口；`44-title-position-default-state-before-after.png` 直接左右拼接为 `2530 × 720 px`，没有缩放、裁切或密度插值。右侧同时展示用户要求的新空白通用初始状态。
...
- 主副标题整体位置：`.hero-copy` 增加 `22 px` 顶部内边距；桌面艺术标题从 `y=18` 下移到 `y=40`，中文副标题从 `y=161` 下移到 `y=183`。`900 px` 下标题、副标题与存档纸条均无相交，手机端随自然文档流增加相同位移。
...
- 文案与内容：删除泛化的长句说明，改为仿照 D6 与合成宝袋道具说明格式的两行短句“窥见你的命运 / Glimpse your destiny”，不加引号与句号；页面不再展示“恢复目标 169”或默认猫套条件，目标 169 只作为仓库回归样例存在；其他筛选说明、版本、道具名称与应用实际状态一致。
...
- 页面初次载入时口袋物类型、口袋物、主动和被动均为空，摘要为“当前没有筛选条件”，DOM 中不存在目标 169 预设按钮。手动选择“饰品”后点击“清空条件”，类型与全部 ID 字段重新归空。
...
15. 用户要求把艺术主标题与双语副标题作为整体下移，并清除面向个人猫套目标的初始值与恢复预设；原状态按 `[P2]` 通用产品初始状态和视觉位置问题处理。
16. 修正：`.hero-copy` 整体下移 `22 px`；删除 HTML 中口袋物、主动、被动的默认值与饰品默认类型；移除“恢复目标 169”按钮及其前端函数、事件和状态逻辑。`44-title-position-default-state-before-after.png` 显示首屏位置与空白状态变化，`41/42/43` 验证三个断点没有新遮挡或溢出，此前 P2 已关闭。
...
整体下移与空白默认状态专项使用 `44-title-position-default-state-before-after.png`：左侧是比例修正后的个人目标状态，右侧是主副标题下移 `22 px` 后的通用空白状态；同尺寸全视图同时覆盖标题位置、预设按钮和首组筛选字段，无需额外局部裁切。
...
- [x] 艺术主标题与双语副标题已整体下移 `22 px`，并通过 1280 / 900 / 390 三个宽度复核。
- [x] WebUI 默认筛选条件已清空，目标 169 恢复按钮及其前端逻辑已删除。
- [x] 后端提供本机地下室图集端点，前端完成响应式房间拼接。
- [x] 桌面、窄屏、空白初始状态、清空条件、控制台和搜索冒烟测试通过。
- [x] 没有剩余 P0 / P1 / P2 问题。
