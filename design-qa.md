...
- 字体与排版：艺术标题保持透明位图；大标题、纸片区块标题和本轮双语副标题使用 LanaPixel；表单、说明、按钮、道具名和表格使用 Segoe UI / Microsoft YaHei UI；种子、ID、品质和统计数值使用 Cascadia Mono / Consolas 等宽栈。双语副标题现以 `22 px` 中文主句和 `14 px` 英文副句建立层级，较上一版的 `18 / 11 px` 均放大一档，继续使用 `1.15` 行高与 `.08em` 字距；桌面、中等宽度和窄屏均无换行、截断或异常回退。
...
- [x] 双语副标题已放大到 `22 / 14 px`，并通过 1280 / 900 / 390 三个宽度复核。
