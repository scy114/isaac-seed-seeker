...
| --- | --- | --- | --- | --- |
| 开局硬币、钥匙、炸弹 | 已在当前分支实现 | RNG 分支已在当前 C++ 内核中执行；上游有明确取值规则 | 低 | 已接入 CLI、HTTP、WebUI、排序和回归测试 |
| 实验性疗法（黄针，ID 240）结算 | 已在当前分支实现，待实机抽样复核 | 上游已有 7 属性洗牌算法；游戏 API 和 Wiki 给出效果语义 | 中 | 已接入方向/结算值筛选；合并前保留实机复核项 |
...
> 2026-08-05 实施状态：用户选择先做前两项。本分支已经落地开局资源和黄针专用后处理器；一层宝箱房与 Boss 奖励仍停留在调研，不进入本次实现。
